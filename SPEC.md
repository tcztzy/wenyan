...
- 忽略符号集合：`"。"`、`"、"`、`"，"`、`"矣"`、空格、`\t`、`\n`、`\r`、`"　"`（全角空格）。
- 默认情况下，这些字符会被跳过。
- 若当前正处于“數據”读取中，遇到忽略符号会**先切出当前數據 token**，再跳过忽略符号。
...
- 数据块会在以下情况被切断并输出：
  ...
  - 遇到忽略符号
  - 到达文件末尾
- 若数据块已在累积，后续连续普通字符会继续并入同一块，直到被切断。
...
## 10. 「其餘」相关补充（语法/执行）
> 本文主轴是词法；以下为与 `其餘` 直接相关、当前实现中的语法与执行行为补充。

### 10.1 取句
- 支持两种形式：
  - `取<正整数>`
  - `取其餘`
- `取其餘` 会把当前暂存栈中的**全部值**交给下一条 `以施`，并清空暂存栈。
- 仍遵守既有约束：
  - `取` 后若不是 `以施`，报 `取後需以施`
  - 语句结束时还有未消费的 `取`，报 `取後未以施`
  - `以施` 前无 `取`，报 `以施需先取`

### 10.2 術参组中的「其餘」
在术定义的 `必先得` 参组里，支持变长参组：

- 形式：`其餘<型別>。曰「名」`
- 约束：
  - 必须且仅能有一个名字，否则报 `其餘參數須一名`
  - 只能出现一次，多次出现报 `其餘參數不可多`
  - 必须位于参组末尾，后面再跟参组时报 `其餘參數須居末`

### 10.3 调用语义
- 当术定义了 `其餘` 参组时：
  - 固定参先按位置绑定；
  - 剩余实参会整体绑定到该 `其餘` 形参（以列传入）。
- 该语义同时适用于：
  - 直接 `施` 调用
  - `取...` / `取其餘` + `以施`
  - 柯里化/部分套用后的继续调用
