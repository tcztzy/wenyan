...
## 核心原则（必须遵守）
- **Python 3.10+ 兼容**：所有代码必须在 Python 3.10 及以上版本 *可解析并可运行*（以 tox 的 `py310` 为底线）。
  - 允许使用较新的 *类型注解写法*（例如 PEP 604 `A | B`、PEP 585 `list[str]`），并可直接使用现代类型系统（例如 `A | B`、`list[str]`）；允许在运行时安全读取注解，但仍应避免把注解当作业务逻辑分支条件。
  - 可使用 Python 3.10+ 语法（例如结构化模式匹配 `match/case`）；若引入 3.11+ 语法需先提升最低版本。
...
- **类型标注**：
  - 以 Python 3.10 为底线：可使用 3.10 语法（含 `match/case`）；若需 3.11+ 语法需先提升最低版本。
  - 类型注解优先使用现代内建写法（如 `A | B`、`list[str]`）；不再要求 `from __future__ import annotations`。
  - 避免新增 `typing` 旧式容器注解（如 `Optional/Union/List/Dict`），除非与第三方类型协议交互不可避免。
- **文档与注释**：
