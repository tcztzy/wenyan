...
- 容器：`列/物`，`充`（push）、`銜`（concat）、`之/之長/之其餘`（下標/長度/切片）
- 物定義：`其物如是 … 是謂 … 之物也`
- for-each：`凡 … 中之 …`
- 匯入：`吾嘗觀 … 之書 … 方悟 … 之義`
...
- `其值()`（取暫存棧頂並清空暫存棧）
- `其餘值()`（只用於 `之其餘` 的 REST 形式；獨立成值會報文法錯）
- `爻值(真: bool)`（陰/陽）
...
#### 控制流（區塊）
- `若句(條件: list[條件原子|str], 反轉: bool, 然: list[句], 另若列: list[或若子句], 否則: list[句])`
  - 條件原子為 `值` 加可選後綴：`之 <值>` 或 `之長`。
  - 比較詞：`等於/不等於/大於/小於/不大於/不小於`，可鏈式轉為 Python `Compare`。
  - 邏輯詞：`中無陰乎` 轉 `&&`，`中有陽乎` 轉 `||`；轉譯時 `&&` 優先於 `||`。
  - `若其然者` 以 `其` 作條件；`若其不然者` 亦以 `其` 作條件並反轉。
  - `或若` 使用同一條件式語法；`若非` 無條件。
...
#### 函數/匯入/容器/物/例外
- `術參數(名: str, 類型: str, 其餘: bool = False)`（`其餘=True` 表示變長參數）
- `術定義句(名: str, 參數列: list[術參數], 體: list[句], 公開: bool)`
- `施句(術: 值, 參數列: list[值])`、`以施句(術: 值)`、`取句(數量: int | None, 其餘: bool)`
...
- 暫存棧：
  ...
  - 由於 `其` 具有「取值並清空」的副作用，轉譯器應產生輔助函式（例如 `__其()`）：
    ...
    - `其` 轉譯為 `__其()`。
  - `取`/`以施` 使用 `__取(n)`；`取其餘` 使用 `__取其餘()` 取出並清空暫存棧。
- 下標（`之` / `昔今`）：
  - 讀取由 `取物` helper 統一處理（`列` 為 1-based；`<=0` 索引走 `__文言負索` 映射）。
  - 賦值保持既有語義（對 `列` 正向越界可補 `None` 再寫入）。
  - 刪除由 `刪物` helper 統一處理：
    - `昔之「名」者。今不復存矣。` 會將該名綁定為 `None`。
    - 對 `列` 的正向刪除會移除該槽位；越界不擴列。
    - 對 `列` 的 `<=0` 索引會清除對應 `__文言負索` 映射。
- 函數呼叫：
...
- JS-only 標準庫等價策略：
  - `lib/js/*.wy` 若要於 Python 目標可用，須在 `lib/py/` 有同名 `.wy` 等價層。
  - Python 目標搜尋匯入時除 `曆法` 外先取 `lib/py`，後取根 `lib`；`曆法` 為相容既有根庫，先取根 `lib`。
  - JS DOM/canvas 等宿主 API 不直接移植；以 Python 無頭相容物件或 no-op 等價層保留可執行語義。
  - 需暴露宿主內建時，置於內建序言（如 `JSON.stringify`、`String.fromCharCode`），仍不得新增 runtime 第三方依賴。
- 例外：
...
- `之句`：
  - 讀取生成 `取物(容器, 索引)`；`之其餘` 生成從文言第 2 項起算的切片。
  - 字串鍵走 dict key / 容器 key / attribute fallback；數字鍵按文言 1-based 轉換。
  - 對 `列` 的 `<=0` 索引與越界讀取，保持既有 `None/負索映射` 行為。
- `昔今句`：
  - 名賦值直接生成 `Assign`；下標賦值生成必要的 `Subscript`、擴列與負索映射邏輯。
  - 下標刪除生成 `刪物(容器, 索引)`；名刪除生成 `name = None`。
  - 對 `列` 的 `>長度` 賦值先擴容；`<=0` 下標寫入/刪除走 `__文言負索` 映射。
- `若句`：
  - 生成 `ast.If(test=..., body=..., orelse=...)`。
  - 每個無邏輯段轉為 Python `Compare` 或單值 truth test；`中無陰乎` 段先併為 `ast.And`，再以 `中有陽乎` 併為 `ast.Or`。
  - 條件後綴 `之` 轉 `取物(值, 索引)`；`之長` 轉 `len(值)`。
- `恆為是句`：
...
- `術定義句`：
  ...
  - 本體與包裝皆附加 `__文言術參數數__`；若有變長參數，另附 `__文言術接其餘__`。
  - 依作用域分析插入 `global` / `nonlocal`
- `施句` / `以施句`：
  - 直接生成 `術(*參數)`，並將結果推入 `__暫存`
- `取句`：
  - 影響下一次 `以施` 的取值模式：固定數量（`取 <數>`）或全部暫存（`取其餘`）
- `返回句`：
  - `乃得 <值>` → `return <值>`
  - `乃得矣` → `return __其()`
  - `乃歸空無` → `return None`
...
- JS-only 標準庫語義目前以 `lib/py` 同名等價層與內建序言補足；若新增 `lib/js/*.wy`，須同步新增 `lib/py/*.wy` 或在本節記明不可支援原因。
- 更高保真 DOM/canvas 輸出仍非目標；Python 端以可測無頭語義為準。
