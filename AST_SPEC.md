...
限制：
- JS-only 標準庫（如 `畫譜`）不在 Python 子集範圍內；其語義不在本規範中定義。
...
- 函數呼叫：
  - 轉譯器提供 `__文言呼叫(術, *參數)`，支援 curry/partial（依 `__文言術參數數__`）。
- 內建：
...
- `術定義句`：
  - 生成 `FunctionDef`，並附加 `__文言術參數數__` 屬性
  - 依作用域分析插入 `global` / `nonlocal`
- `施句` / `以施句`：
  - 生成 `__文言呼叫(術, *參數)`，並將結果推入 `__暫存`
- `取句`：
