...
class 自舉準備測試(unittest.TestCase):
    def _載入自舉作用域(self) -> dict[str, Any]:
        ...
        作用域: dict[str, Any] = {"__name__": "__main__", "__file__": str(路徑)}
        exec(程式碼, 作用域, 作用域)
        return 作用域
    ...
    def test_分詞言值對齊_host_轉義與巢狀(self) -> None:
        作用域 = self._載入自舉作用域()
        分詞 = 作用域["分詞"]
        self.assertTrue(callable(分詞))

        記號列 = 分詞('曰「「甲\n"乙」」。曰「「星」」」。曰『甲『乙』丙』。')
        言列 = [記 for 記 in 記號列 if 記["類"] == "言"]

        self.assertEqual(
            [記["文"] for 記 in 言列], ['甲\\n\\"乙', "星」", "甲『乙』丙"]
        )

    def test_分詞記號止位可還原原文片段(self) -> None:
        作用域 = self._載入自舉作用域()
        分詞 = 作用域["分詞"]
        self.assertTrue(callable(分詞))
        ...
        來源 = "吾有一數。曰二。名之曰「甲」。"
        記號列 = 分詞(來源)
        片段列 = [來源[記["始"] - 1 : 記["止"] - 1] for 記 in 記號列]

        self.assertEqual(片段列, ["吾有", "一", "數", "曰", "二", "名之曰", "「甲」"])

    def test_分詞保留註釋與宏關鍵詞(self) -> None:
        作用域 = self._載入自舉作用域()
        分詞 = 作用域["分詞"]
        self.assertTrue(callable(分詞))

        記號列 = 分詞(
            "注曰「「甲」」。疏曰「「乙」」。批曰「「丙」」。或云「「丁」」。蓋謂「「戊」」。"
        )
        文列 = [記["文"] for 記 in 記號列]
    ...
    def test_自舉文法骨架可析夫與返回句(self) -> None:
        ...
        句列 = 析句列(記號列)
        self.assertEqual(
            [句["類"] for 句 in 句列], ["夫句", "返回句", "返回句", "返回句"]
        )
    ...
    def test_自舉文法骨架可析昔今句下標(self) -> None:
        ...
        記號列 = 分詞(
            "昔之「甲」之一者。今「乙」之二是矣。昔之「丙」之三者。今不復存矣。"
        )
        句列 = 析句列(記號列)
        self.assertEqual([句["類"] for 句 in 句列], ["昔今句", "昔今句"])
    ...
    def test_自舉最小執行器可行恆為是與乃止(self) -> None:
        ...
        結果 = 解譯(
            "吾有一數。曰零。名之曰「甲」。恆為是。昔之「甲」者。今三是矣。乃止。云云。乃得「甲」。"
        )
    ...
    def test_自舉文法骨架可析術定義句(self) -> None:
        ...
        記號列 = 分詞(
            "今有一術。名之曰「恆三」。欲行是術。乃行是術曰。乃得三。是謂「恆三」之術也。"
        )
        句列 = 析句列(記號列)
        self.assertEqual([句["類"] for 句 in 句列], ["術定義句"])
    ...
    def test_自舉文法骨架可析算術與變句(self) -> None:
        ...
        句列 = 析句列(記號列)
        self.assertEqual(
            [句["類"] for 句 in 句列], ["算術句", "算術句", "算術句", "算術句", "變句"]
        )
    ...
    def test_自舉最小執行器可行大單位數值(self) -> None:
        作用域 = self._載入自舉作用域()
        解譯 = 作用域["解譯"]
        self.assertTrue(callable(解譯))
        ...
        for 程式, 期 in 例列:
            with self.subTest(程式=程式):
                self.assertEqual(解譯(程式), 期)
    ...
    def test_自舉最小執行器可返回與昔今(self) -> None:
        ...
        結果一 = 解譯(
            "吾有一數。曰一。名之曰「甲」。昔之「甲」者。今二是矣。乃得「甲」。"
        )
        ...
        結果三 = 解譯(
            "吾有一數。曰一。名之曰「甲」。昔之「甲」者。今不復存矣。乃得「甲」。"
        )
